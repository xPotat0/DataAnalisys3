...
### Ход работы:
- Выберем шар RollerAgent и уберём модель поведения в компоненте Behaviour Parameters. Поставим Inference Device и Behavior Type на настройки "Default"
...
После всех манипуляций с кодом скрипт-файл будет выглядеть вот так:
...
```
- Нажмём на шар. Теперь у RollerAgent в компоненте скрипт появилось новое поле для Target_2. "Перетащим" в это поле синий шар Target_2.

![22](https://user-images.githubusercontent.com/106258306/197498512-e3557904-5c40-4c24-99c5-fc93474a2c72.png)

- Перейдём в консоль и запустим новый MLAgent. Запустим сцену. Подождём некоторое время и остановим сцену. Создадим ещё 3 площадки Target_area и возобновим работу MLAgent. Снова запустим сцену.

![20](https://user-images.githubusercontent.com/106258306/197499034-1fdaec08-8cfd-4417-a952-550c41627781.png)

- Подождём некоторое кол-во времени о остановим сцену. Проверим работу созданного агента. 
- Зайдём в папку с проектом, перейдём в папку result и возьмём оттуда файл с названием RollerBall.onnx. Переименуем его на RollerBall_1.onnx и Скопируем его.
- В компоненте Behaviour Parameters *(У шара RollerAgent)* установим настройку Model. Перетащим наш файл RollerBall_1.onnx, который мы скопировали ранее в папку assets, и перетащим его в Model.
- Установим настройку Inference Device = CPU.
- Установим настройку Behavior Type = Inference Only.
- Уберем лишнее TargetArea и запустим сцену. Шар стал пытаться переместиться в точку между двумя кубами.
